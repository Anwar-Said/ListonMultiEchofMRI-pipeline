...
source /isis/home/saida2/anaconda3/etc/profile.d/conda.sh   ## add path to anaconda under func_denoise_meica.sh file line #42
conda activate me_v10 
...
func () {
	...
	# tedana -d "/isis/home/saida2/projects/Conorpipeline/data/WEL002/func/rest/session_1/run_1/Rest_E1_acpc.nii.gz" -e $(cat "/isis/home/saida2/projects/Conorpipeline/data/WEL002/func/rest/session_1/run_1/TE.txt)" --out-dir "/isis/home/saida2/projects/Conorpipeline/data/WEL002/func/rest/session_1/run_1/Tedana/"
	# --tedpca "tdl" --fittype curvefit --mask "/isis/home/saida2/projects/Conorpipeline/data/WEL002/func/rest/session_1/run_1/brain_mask.nii.gz" --maxit "500" --maxrestart "5" --seed 42
	# exit 1
	...
	# mv "$1"/func/rest/"$6"/Tedana/desc-optcom_bold.nii.gz "$1"/func/rest/"$6"/Rest_OCME.nii.gz # optimally combined time-series;
	# mv "$1"/func/rest/"$6"/Tedana/desc-optcomDenoised_bold.nii.gz "$1"/func/rest/"$6"/Rest_OCME+MEICA.nii.gz # multi-echo denoised time-series;
	...
	# for i in desc-ICA_components t2sv s0v ; do
...
# sweep the sessions;
for s in $sessions ; do
	# echo -e "under the sessions loop"
	...
	for r in $runs ; do
		...
		cp -rf "$MEDIR"/res0urces/auto_screen_parse_man_rej.m \
...
# echo -e "finish looping on sessions"
# define a list of directories;
...
func () {
	...
	--tedpca "$3" --fittype curvefit --mask "$1"/func/rest/"$6"/brain_mask.nii.gz --mix "$1"/func/rest/"$6"/Tedana/ica_mixing.tsv \
	...
	# mv "$1"/func/rest/"$6"/Tedana+ManualComponentClassification/desc-optcom_bold.nii.gz "$1"/func/rest/"$6"/Rest_OCME.nii.gz # optimally combined time-series;
	# mv "$1"/func/rest/"$6"/Tedana+ManualComponentClassification/desc-optcomDenoised_bold.nii.gz "$1"/func/rest/"$6"/Rest_OCME+MEICA.nii.gz # multi-echo denoised time-series;
...
# echo -e "running the last function!!!"
export -f func # run tedana;
parallel --jobs $NTHREADS func ::: $Subdir ::: $Subject ::: $MEPCA ::: $MaxIterations ::: $MaxRestarts ::: $DataDirs #> /dev/null 2>&1
